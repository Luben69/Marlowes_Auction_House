class Profile:
    ...
    def password(self, value):
        ...
        contains_upper_latter = any([el for el in value if el.isupper()])
        contains_digit = any([el for el in value if el.isdigit()])

        if not is_long_enough or not contains_upper_latter or not contains_digit:
            raise ValueError("The password must be "
                             "8 or more characters with at least "
